...
namespace lucidy{
    ...
    void FeatureDetector::detectFeatures(Image & sourceImage){
        /// detect features in image and put them in image featureList
        /// if image featureList has already been used a.k.a. is full remove all the content 

        if ( sourceImage.features.size() > 0 ){ sourceImage.features.clear(); };  
        detector->detect(sourceImage.get(), sourceImage.features, cv::Mat() );
